...
function decreaseTime() {
    if(time === 0) {
        timeOut.classList.remove('red')
        endGame()
    }else {
        let currentTime = --time;
        if(currentTime < 10) {
            currentTime = '0' + currentTime
            timeOut.classList.add('red')
        }
...
function createBall() {
    ...
    ball.classList.add(figures[random(0,3)])
    ball.style.background = colors[random(0,2)]
...
let figures = ['square', 'triangle', 'circle', 'star']